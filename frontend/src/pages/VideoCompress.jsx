...
export default function VideoCompress() {
  ...
  const handleFiles = files => {
    ...
    if (!user) {
      ...
      setVideos(prev => [...prev, ...smallFilesArr]);
    } else {
      // Logged-in users: no limit
      setVideos(prev => [...prev, ...fileArr]);
    }
  ...
  return (
    <main className="videocompress-main">
      <section className="videocompress-card">
        <div className="videocompress-header">
          <span className="videocompress-icon" aria-hidden="true">
            {/* Video SVG */}
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <rect x="3" y="5" width="18" height="14" rx="3" fill="#6c63ff" fillOpacity="0.13"/>
              <polygon points="10,9 16,12 10,15" fill="#6c63ff" fillOpacity="0.22"/>
            </svg>
          </span>
          <h1 className="videocompress-title">Video Compressor</h1>
          <p className="videocompress-desc">Compress your videos quickly and efficiently. Upload, compress, and download all in one place.</p>
        </div>
        ...
        <div
          className="videocompress-upload"
        ...
        >
          ...
          <span className="upload-text">Drag & drop or <span className="upload-link">browse</span> videos</span>
            ...
            {videos.map((vid, idx) => (
              <div className="video-item" key={idx}>
                ...
                <div className="video-info">
                  <span className="video-name">{vid.name}</span>
                  <span className="video-size">{formatFileSize(vid.size)}</span>
                </div>
                <button className="video-remove" onClick={e => { e.stopPropagation(); handleRemove(idx); }} title="Remove">
                  {/* X SVG */}
                  ...
                  {result.error ? (
                    ...
                    <>
                      <div className="comparison-container">
                        ...
                        {/* Compressed Video */}
                        <div className="video-comparison">
                          <div className="video-label">Compressed</div>
                          <div className="video-preview-container">
                            <video 
                              src={URL.createObjectURL(result.originalFile)} 
                              className="comparison-video"
                      ...
                      <div className="compression-stats-card">
                        ...
                        {/* Enhancement details hidden from user */}
                      </div>
