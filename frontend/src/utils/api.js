...
export const compressionAPI = {
  ...
  compressVideo: async (file) => {
    try {
      ...
      console.log('Sending request to:', `${API_BASE_URL}/compress/video`);
      ...
      // Handle both compression and fallback responses
      if (result.success) {
        return result;
      } else {
  ...
  // Video enhancement - improves quality without size increase
  enhanceVideo: async (file) => {
    try {
      console.log('Starting video enhancement for:', file.name);
      ...
      console.log('Sending enhancement request to:', `${API_BASE_URL}/compress/video/enhance`);
      
      const response = await fetch(`${API_BASE_URL}/compress/video/enhance`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        console.error('Enhancement server error:', errorData);
        throw new Error(errorData.error || `HTTP error! status: ${response.status}`);
      }
